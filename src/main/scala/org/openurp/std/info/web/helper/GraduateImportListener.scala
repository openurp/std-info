...
class GraduateImportListener(entityDao: EntityDao, project: Project) extends ImportListener {
  ...
  override def onItemStart(tr: ImportResult): Unit = {
    transfer.curData.get("std.code") foreach { code =>
      ...
      val stds = entityDao.search(query).headOption match {
        ...
        case Some(std) =>
          transfer.curData.put("graduate.std", std)
          entityDao.findBy(classOf[Graduate], "std", std).headOption foreach { g =>
            transfer.current = g
          }
      }
    }
