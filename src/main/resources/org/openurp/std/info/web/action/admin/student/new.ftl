...
<style>
  ...
  [@b.form name="studentAddForm" action="!saveAddInfo" theme="list"]
    ...
    [#if squadSupported]
    [@b.select label="班级" name="state.squad.id" items=[] empty="..." value="" style="width: 200px" comment="（填了年级、培养层次、学生类别、专业所在院系、专业后，才可选；若仍空，则表示无班级可填）"/]
    [/#if]
    [@b.startend label="入学~预计离校" name="student.beginOn,student.endOn" required="true" required="true" style="width: 120px"/]
    [@b.date label="预计毕业日期"  name="student.graduateOn" required="true" value="" style="width: 120px"/]
    [@b.date label="最晚离校日期"  name="student.maxEndOn" required="true" value="" style="width: 120px"/]
