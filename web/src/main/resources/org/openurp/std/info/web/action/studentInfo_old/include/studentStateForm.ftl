...
[@b.form action="!studentStateSave" name="studentStateForm" target="_self" theme="list"]
  ...
  [#if stateRestricts?seq_contains("grade") || !isEdit]
    [@b.textfield label="年级" name="state.grade" value=(state.grade)! maxlength="7" required="true" style="width: 200px"/]
  [#else]
    [@b.field label="年级"]${state.grade}[/@]
    [#assign disallowCount = 0/]
  [/#if]
  [#assign s = "state_"/]
  [#if stateRestricts?seq_contains("department") || !isEdit]
  ...
  [#else]
    [@b.field label="院系"][@i18nName state.department/][/@]
    [#assign disallowCount = 1/]
  [/#if]
  ...
  [#if stateRestricts?seq_contains("department") || stateRestricts?seq_contains("major") || !isEdit]
    [@b.select id=s + "major" label="专业" name="state.major.id" items=[] empty="..." value=(state.major.id)! style="width: 200px"/]
  [#else]
    [@b.field label="专业"][@i18nName state.major/][/@]
    [#assign disallowCount = 2/]
  [/#if]
  [#if stateRestricts?seq_contains("department") && stateRestricts?seq_contains("major") || stateRestricts?seq_contains("direction") || !isEdit]
    [@b.select id=s + "direction" label="方向" name="state.direction.id" items=[] empty="..." value=(state.direction.id)! style="width: 200px"/]
  [#else]
    [@b.field label="方向"][@i18nName (state.direction)!/][/@]
    [#assign disallowCount = 3/]
  [/#if]
  [#if stateRestricts?seq_contains("grade") && stateRestricts?seq_contains("department") && stateRestricts?seq_contains("major") && stateRestricts?seq_contains("direction") || stateRestricts?seq_contains("squad") || !isEdit]
  ...
  [#else]
    [@b.field label="行政班级"]${(state.squad.name)!}[/@]
    [#assign disallowCount = 4/]
  [/#if]
  ...
  [/#switch]
  [#if stateRestricts?seq_contains("inschool") || !isEdit]
    [@b.radios label="是否在校" name="state.inschool" value=(state.inschool?string(1, 0))!/]
  [#else]
    [@b.field label="是否在校"]${state.inschool?string("是", "否")}[/@]
  [/#if]
  [#if stateRestricts?seq_contains("status") || !isEdit]
    [@b.select label="学籍状态" name="state.status.id" items=statuses empty="..." value=(state.status.id)! style="width: 200px"/]
  [#else]
    [@b.field label="学籍状态"][@i18nName state.status/][/@]
  [/#if]
  [#if stateRestricts?seq_contains("campus") || !isEdit]
    [@b.select label="校区" name="state.campus.id" items=campuses empty="..." required="true" value=(state.campus.id)! style="width: 200px"/]
  [#else]
    [@b.field label="校区"][@i18nName (state.campus)!/][/@]
  [/#if]
  [#if stateRestricts?seq_contains("beginOn") && stateRestricts?seq_contains("endOn") || !isEdit]
    ...
    [@b.validity]
      $(":text[name='state.beginOn']", document.studentStateForm).require().assert(function(element) {
        ...
        if (isCloseObj.length == 0 || !isBlank(isCloseObj.val())) {
          return true;
        }
        ...
        for (var i = 0; i > -1; i++) {
          ...
          if (isBlank(iStateId) && isBlank(iStateGrade)) {
            break;
          }
        ...
        $.ajax({
          "type": "post",
          "url": "${base}/student!stateDateCheckAjax.action",
          "dataType": "json",
          "data": stateDataMap,
          "async": false,
    ...
    [/@]
  [#else]
    [@b.field label="起始日期"]${(state.beginOn)?string("yyyy-MM-dd")}[/@]
    [@b.field label="结束日期"]${((state.endOn)?string("yyyy-MM-dd"))!}[/@]
  [/#if]
  [#if stateRestricts?seq_contains("remark") || !isEdit]
  ...
  [#else]
    [@b.field label="备注"]${(state.remark?html?replace("\n", "<br>"))!"<br>"}[/@]
  [/#if]
  ...
  [@b.formfoot]
    ...
    <button id="btnStateClose" type="button">关闭</button>
  [/@]
  ...
  <script>
    $(function() {
      ...
      formObj.find("select[name='state.squad.id']").focus(function() {
        ...
        if (isBlank(grade) || isBlank(projectId) || isBlank(levelId) || isBlank(stdTypeId) || isBlank(departmentId) || isBlank(majorId)) {
          return false;
        }
        ...
        $.ajax({
          "type": "post",
          "url": "${base}/student!loadSquadAjax.action",
          "dataType": "json",
          "data": {
            "grade": grade,
            "projectId": projectId,
      ...
      [#if stateRestricts?seq_contains("beginOn") && stateRestricts?seq_contains("endOn")]
      var spanObj = $("<span>");
