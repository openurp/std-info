...
<table style="width:95%" align="center" class="infoTable">
    ...
    <tr>
      ...
      <td ${ContentTdHTML1}>${(basicInfo.oldname)!}</td>
      <td class="title"${titleTdHTML}>民族：</td>
        <td ${ContentTdHTML2}>${(basicInfo.nation.name)!}</td>
    </tr>
    <tr>
        <td class="title"${titleTdHTML}>政治面貌：</td>
        <td>${(basicInfo.politicVisage.name)!}</td>
        <td class="title" ${titleTdHTML}>出生年月：</td>
        <td>${(basicInfo.birthday?string("yyyy-MM-dd"))!}</td>
    </tr>
    <tr>
      <td class="title"${titleTdHTML}>证件类型：</td>
        <td>${(basicInfo.idType.name)!}</td>
        <td class="title"${titleTdHTML}>证件号码：</td>
        <td>${(basicInfo.idcard)!}</td>
    </tr>
    <tr>
      <td class="title"${titleTdHTML}>籍贯：</td>
        <td>${(basicInfo.ancestralAddr)!}</td>
       <td class="title"${titleTdHTML}>国家地区：</td>
        <td>${(basicInfo.country.name)!}</td>
    </tr>
    <tr>
      <td class="title" ${titleTdHTML}>婚姻状况：</td>
        <td>${(basicInfo.maritalStatus.name)!}</td>
      <td class="title" ${titleTdHTML}>入团(党)时间：</td>
      <td>${(basicInfo.joinOn?string("yyyy-MM-dd"))!}</td>
    </tr>
    <tr>
      <td class="title" ${titleTdHTML}>开户名：</td>
        <td>${(basicInfo.bankAccount.name)!}</td>
      <td class="title" ${titleTdHTML}>开户银行：</td>
      <td>${(basicInfo.bankAccount.bank)!}</td>
    </tr>
    <tr>
      <td class="title" ${titleTdHTML}>银行账号：</td>
        <td>${(basicInfo.bankAccount.account)!}</td>
    </tr>
</table>
</#if>
