...
import org.openurp.std.info.helper.MajorStudentImportListener
...
class MajorStudentAction extends RestfulAction[MajorStudent] with ProjectSupport {
  ...
  override protected def saveAndRedirect(entity: MajorStudent): View = {
    if (!entity.persisted) {
      ...
      if (stds.isEmpty) return forward("edit", "学号不存在")
      val std = stds.head
      val mss = entityDao.findBy(classOf[MajorStudent], "std", stds)
      if (mss.nonEmpty) return forward("edit", "主修信息已经存在")
      entity.std = std
    }
  ...
  def downloadTemplate(): Any = {
    ...
    sheet.add("学号", "majorStudent.std.user.code").length(100).required()
