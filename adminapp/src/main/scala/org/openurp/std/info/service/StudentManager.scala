...
import org.openurp.base.edu.model.{Squad, Student, StudentState}
import org.openurp.base.edu.service.StudentService
import org.openurp.base.edu.service.event.CreateStudentEvent
...
object StudentManager {
  ...
  propertyMap.put("department", "行政管理院系")
...
class StudentManager extends BaseServiceImpl {
  ...
  def checkAndRemoveLogForStudentAlteration(stdAlterations: Iterable[StdAlteration], ip: String, user: User): Unit = {
    ...
    stdAlterations.foreach(stdAlteration => {
      ...
      log.operation = "已删除预定于" + stdAlteration.beginOn + "对学号为" + stdAlteration.std.user.code + "的未生效学籍异动[" + stdAlteration.alterType.name + "]"
      log.user = user
      ...
      log2.operation = "已删除预定于" + stdAlteration.beginOn + "对学号为" + stdAlteration.std.user.code + "的未生效学籍异动[" + stdAlteration.alterType.name + "]"
      log2.user = user
  ...
  //    if (null == before) {
  //      addGeneralLog(new StudentLogParameter(0, "新增了学号为" + std.user.code + "的基本信息", ip, std, loginUser))
  //      return
  //    }
  //    val changedDatas = doCheckDifference(before, now)
  //    if (changedDatas.size < 1) {
  ...
  //    saveDatas.+=(addGeneralLog(new StudentLogParameter(0, "修改了一条学号为<a target='_blank' title='查看详情' href='studentLog!stdLogList.action?student.id=" + std.id + "'>" + std.user.code + "</a>的基本信息", ip, std, loginUser)))
  //    entityDao.saveOrUpdate(saveDatas)
  //  }
...
//  }
  ...
  def getStudentDatasById(stdId: Long): mutable.Map[String, Any] = {
    ...
    if (0L != stdId) {
      ...
      params.put("inSchool", studentService.isInschool(student))
      ...
      val graduations = studentInfoService.getStudentInfo(classOf[Graduation], student)
      if (!graduations.isEmpty) {
        params.put("graduation", graduations(0))
      }
      params.put("stdStatus", studentService.getStdStatus(student))
      // params.put("stdExtInfo",
      // studentInfoService.getStudentInfo(StdExtInfoBean.class,
      // student));
