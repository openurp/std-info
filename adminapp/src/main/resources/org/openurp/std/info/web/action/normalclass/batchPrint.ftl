<@b.head/>
<#macro i18nName(entity)><#if locale.language?index_of("en")!=-1><#if entity.enName!?trim=="">${entity.name!}<#else>${entity.enName!}</#if><#else><#if entity.name!?trim!="">${entity.name!}<#else>${entity.enName!}</#if></#if></#macro>
<style>
.printTableStyle {
...
<@b.toolbar title="班级学生名单打印">
   ...
   <#list normalClasses as normalClass >
   <#assign boycount=0/>
   <#assign girlcount=0/>
    <#assign stds=stdMap[normalClass.id?string]>
    <br>
     ...
     <tr><td>&nbsp;</td></tr>
     <tr>
       <td colSpan="10" style="font-size:12pt" id="class${normalClass.id}">班级：<@i18nName normalClass?if_exists/></td>
     </tr>
   </table>
   ...
   <table class="printTableStyle"  width="100%"  >
     ...
     <script>
         var titletd=document.getElementById("class${normalClass.id}");
